...
@Controller
public class CheckValidationApiController implements CheckValidationApi {

    private static final Logger log = LoggerFactory.getLogger(CheckValidationApiController.class);
    ...
    public CheckValidationApiController(ObjectMapper objectMapper, HttpServletRequest request) {
        this.objectMapper = objectMapper;
        this.request = request;
    }

    public ResponseEntity<InlineResponse200> checkValidation(@NotNull @ApiParam(value = "user name to check if validation allowed for", required = true) @Valid @RequestParam(value = "user name", required = true) String userName) {    	
